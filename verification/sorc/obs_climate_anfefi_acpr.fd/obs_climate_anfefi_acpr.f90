...
!     if (ffmon(2).eq.0.0) ffmon(2) = 0.01D0
      ...
      if(ij.eq.6801) then
        print *,  fopara(1), fopara(2),opara(1),opara(2)
      endif
...
!  output EFI
  ...
  print *, '----- Output EFI -----'
  print *, 'gfldo%ipdtnumagfldo%ipdtnum',gfldo%ipdtnum, gfldo%ipdtlen

! if(gfldo%ipdtnum.eq.1) ipdtnum_out=2
! if(gfldo%ipdtnum.eq.11) ipdtnum_out=12

! ipdtnum_out=gfldo%ipdtnum
