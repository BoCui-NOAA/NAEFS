...
tmax_nfhr_00="    030 054 078 102 126 150 174 198 222 246 270 294 318 342 366 384"
tmax_nfhr_06="024 048 072 096 120 144 168 192 216 240 264 288 312 336 360 384"
tmax_nfhr_12="    042 066 090 114 138 162 186 210 234 258 282 306 330 354 378"
tmax_nfhr_18="    036 060 084 108 132 156 180 204 228 252 276 300 324 348 372"

tmin_nfhr_00="018 042 066 090 114 138 162 186 210 234 258 282 306 330 354 378"
tmin_nfhr_06="    036 060 084 108 132 156 180 204 228 252 276 300 324 348 372"
tmin_nfhr_12="    030 054 078 102 126 150 174 198 222 246 270 294 318 342 366 384"
tmin_nfhr_18="024 048 072 096 120 144 168 192 216 240 264 288 312 336 360 384"
...
for acyc in $tmaxlist; do
  afile=$COM_RTMA/rtma2p5.$PDYm2/rtma2p5.t${acyc}z.2dvaranl_ndfd.grb1_ext      
  if [ $acyc -eq 00 ]; then
    afile=$COM_RTMA/rtma2p5.$PDYm1/rtma2p5.t${acyc}z.2dvaranl_ndfd.grb1_ext      
  fi
...
# ofile1=ge${nens}.t${cyc}z.ndgd2p5_conus_mef${nfhrs}_tmax
# ofile2=ge${nens}.t${cyc}z.ndgd2p5_conus_amef${nfhrs}_tmax

  ofile1=${id}.t${cyc}z.conus_ext_2p5_mef${nfhrs}_tmax
  ofile2=${id}.t${cyc}z.conus_ext_2p5_amef${nfhrs}_tmax

# ofile3=rtma_conus_${PDYm2}_tmax
  ofile3=rtma_conus_${PDYm1}_tmax
...
# startmsg
  $EXECcrps/$pgm  <input.$nfhrs.$nens > $pgmout.$nfhrs.${nens}_tmax
# export err=$?;err_chk
...
# cfile=$COM/gefs.${fymd}/${cyc}/ndgd/ge$nens.t${cyc}z.ndgd2p5_conusf${nfhrs}_ext
  COMNDGD=$COM_NDGD/${id}.${fymd}/${cyc}/ndgd
  cfile=$COMNDGD/${id}.t${cyc}z.geavg.f${nfhrs}.conus_ext_2p5.grib1
...
# ofile1=ge${nens}.t${cyc}z.ndgd2p5_conus_mef${nfhrs}_tmin
# ofile2=ge${nens}.t${cyc}z.ndgd2p5_conus_amef${nfhrs}_tmin

  ofile1=${id}.t${cyc}z.conus_ext_2p5_mef${nfhrs}_tmin
  ofile2=${id}.t${cyc}z.conus_ext_2p5_amef${nfhrs}_tmin

# ofile3=rtma_conus_${PDYm2}_tmin
  ofile3=rtma_conus_${PDYm1}_tmin
...
# startmsg
  $EXECcrps/$pgm  <input.$nfhrs.$nens > $pgmout.$nfhrs.${nens}_tmin
# export err=$?;err_chk
