...
if [ "$ENSIM" = "20s" -o "$ENSIM" = "20x" ]; then
   ...
   ex=. 
elif [ "$ENSIM" = "gefs_raw" ]; then
   ex=  
elif [ "$ENSIM" = "ecmwf" ]; then
   ex=
elif [ "$ENSIM" = "gefs_bc" ]; then
   ex=_bc
else
   ex=_bc
fi
...
export pgm=naefs_crps_from_avgspr 
...
###
  ...
  if [ "$runini" = "YES" ]; then
    ...
    if [ $rem -eq 0 ]; then
      ...
      COMANL=$COM/gefs.${aymd}/${acyc}/pgrba
      ...
      COMANL=$COM/gefs.${aymd}/${acyc}/pgrba
      afile1=gegfs.t${acyc}z.pgrbaf03                            
    fi
...
#   afile1=gegfs.t${acyc}z.pgrbaf00                            
#   afile1=gegfs.t${acyc}z.pgrba.0p50f00                            
# fi
  ...
  COMFCST=$COM/${id}.${fymd}/${fcyc}/pgrba$ex
  if [ "$ENSIM" = "gefs_rawp5" ]; then
    COMFCST=$COM/${id}.${fymd}/${fcyc}/pgrba
  fi
...
###
  ...
  cfile1=$COMFCST/${px}geavg.t${fcyc}z.pgrba${ex}f${nfhrs}

###
# forecast files entry, ensembel spread 
###

  cfile2=$COMFCST/${px}gespr.t${fcyc}z.pgrba${ex}f${nfhrs}
