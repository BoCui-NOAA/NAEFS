...
if [ $model = "gefs" ]; then
  ...
  export COMIN=$COMROOT/${model}/${model_ver}/$package.$PDY/$cyc/pgrb2ap5${px}
  export COMOUT=$COMROOT/${model}/${model_ver}/$package.$PDY/$cyc/pgrbap5${px}
fi
export DATA=$DATAROOT/dir_cnvgrib_${model}.${model_ver}.$package.pgrb2ap5${px}.$PDY$cyc
...
for nens in $outlist; do
  ...
  for nfhrs in $hourlist; do
    ...
    if [ $model = "gefs" ]; then
      ...
      file_in=$COMIN/${nens}.t${cyc}z.pgrb2a.0p50${px}f${nfhrs}
      file_out=$COMOUT/${nens}.t${cyc}z.pgrba.0p50${px}f${nfhrs}
    fi
...
set +x
echo " "
echo "Leaving sub script gefs_bcp5_cnvgrib.sh"
echo " "
set -x
