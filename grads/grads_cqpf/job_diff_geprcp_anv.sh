...
export hourlist="048"
export hourlist="048 072 120 240 360 "
export hourlist="054 060 066"                     
...
#while [ $iday -le $ndays ]; do
  ...
  for mem in $memberlist; do 
      ...
      version=v6.1
      rm $version.ctl
      infile=ge${mem}.t${cyc}z.pgrb2a.0p50.anvf${nfhrs}
      cp $COM1/gefs.$PDY/$cyc/prcp_bc_gb2/$infile ${infile}_$version
      $g2ctl -verf ${infile}_$version > $version.ctl
      ...
      sed -e "s/PDY/$PDY/" \
        ...
        -e "s/MMDD/$MMDD/" \
        -e "s/var/$var/" \
        diff_geprcp_anv.gs  >grads.gs

       grads -bcl "grads.gs"

       mv enscqpf.png  diff_geprcp_anv.${PDY}${cyc}.f${nfhrs}.png
    done
  done
