  ...
  for mem in $memberlist; do 
    for var in $varlist; do
      ...
      sed -e "s/cyc/$cyc/" \
        ...
        -e "s/version/$version/" \
        -e "s/nfhrs/$nfhrs/" \
        control_enspqpf_bc.ctl >$version.ctl    
#     $g2ctl -verf $infile  > me.ctl
      ...
      sed -e "s/var/$var/" \
       ...
       mv enscqpf.png  diff_enspqpf_bc.${INTER}.${PRCP}mm.${PDY}${cyc}.f${nfhrs}.png
    done
  done
  done
  done
