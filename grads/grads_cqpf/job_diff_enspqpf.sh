...
#while [ $iday -le $ndays ]; do
  ...
  for nfhrs in $hourlist; do
    for mem in $memberlist; do 
      ...
      version=v6.1
      rm $version.ctl
      infile=ge${mem}.t${cyc}z.pgrb2a.0p50.24hf${nfhrs}
      cp $COM1/gefs.$PDY/$cyc/prcp_gb2/$infile ${infile}_$version
      sed -e "s/cyc/$cyc/" \
        -e "s/INTER/$INTER/" \
        -e "s/version/$version/" \
        -e "s/nfhrs/$nfhrs/" \
        control_enspqpf.ctl >$version.ctl    
#     $g2ctl -verf  ${infile}_$version  > me.ctl
      /u/$LOGNAME/xbin/gribmap -i $version.ctl

      version=v7.0
      rm $version.ctl
      infile=ge${mem}.t${cyc}z.pgrb2a.0p50.24hf${nfhrs}
      cp $COM2/gefs.$PDY/$cyc/prcp_gb2/$infile ${infile}_$version
      sed -e "s/cyc/$cyc/" \
        -e "s/INTER/$INTER/" \
        -e "s/version/$version/" \
        -e "s/nfhrs/$nfhrs/" \
      ...
      sed -e "s/var/$var/" \
       ...
       mv enscqpf.png      diff_enspqpf_${PRCP}mm.${PDY}${cyc}.f${nfhrs}.png
    done
  done
  done
