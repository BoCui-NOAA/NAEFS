...
#while [ $iday -le $ndays ]; do
  ...
  for mem in $memberlist; do 
      ...
      version=v6.1
      rm $version.ctl
      infile=ge${mem}.t${cyc}z.pgrb2a.0p50.bc_${INTER}f${nfhrs}
      cp $COM1/gefs.$PDY/$cyc/prcp_bc_gb2/$infile ${infile}_$version
      $WGRIB2 ${infile}_$version | grep "ENS=+20" | $WGRIB2 -i ${infile}_$version -append -grib mean_${infile}_$version
      ...
      rm $version.ctl
      infile=ge${mem}.t${cyc}z.pgrb2a.0p50.bc_${INTER}f${nfhrs}
      cp $COM2/gefs.$PDY/$cyc/prcp_bc_gb2/$infile ${infile}_$version
#     $WGRIB2 -match "ENS=+20" ${infile}_$version  -append -grib  mean_${infile}_$version
      $WGRIB2 ${infile}_$version | grep "ENS=+20" | $WGRIB2 -i ${infile}_$version -append -grib mean_${infile}_$version
      $g2ctl -verf mean_${infile}_$version > $version.ctl
      /u/$LOGNAME/xbin/gribmap -i $version.ctl
      ...
      sed -e "s/PDY/$PDY/" \
        ...
        -e "s/var/$var/" \
        -e "s/INTER/$INTER/" \
        diff_geprcp_bc.gs  >grads.gs

       grads -bcl "grads.gs"

       mv enscqpf.png  diff_geprcp_bc_mem20.${INTER}.${PDY}${cyc}.f${nfhrs}.png
    done
  done
  done
...
dir_new=diff_geprcp_bc_member.$CDATE  
