...
for nfhrs in $hourlist; do
   ...
   for naefs_ver in v7.0 v6.1; do

   COMIN=$COMROOT/naefs/${naefs_ver}/${id}.$YMD/$cyc/prcp_bc_gb2

   cp $COMIN/${px}geprcp.t00z.pgrb2a.0p50.anvf${FHR3d} anvf${FHR3d}.${id}.${naefs_ver}
   cp $COMIN/${px}geprcp.t00z.pgrb2a.0p50.efif${FHR3d} efif${FHR3d}.${id}.${naefs_ver}

   g2ctl -verf anvf${FHR3d}.${id}.${naefs_ver} > anvf${FHR3d}.${id}.${naefs_ver}.ctl 
   gribmap -i  anvf${FHR3d}.${id}.${naefs_ver}.ctl

   g2ctl -verf efif${FHR3d}.${id}.${naefs_ver} > efif${FHR3d}.${id}.${naefs_ver}.ctl 
   gribmap -i  efif${FHR3d}.${id}.${naefs_ver}.ctl
   ...
   cp $COMIN/geprcp.t00z.pgrb2a.0p50.efif000 ccpa24.t12z.pgrb2a.0p50_efi
   ...
   g2ctl -verf  ccpa24.t12z.pgrb2a.0p50_efi > ccpa24.t12z.pgrb2a.0p50_efi$FHR3d.ctl
   gribmap -i ccpa24.t12z.pgrb2a.0p50_efi$FHR3d.ctl
   ...
   sed -e "s/test/$CDATE/" \
       ...
       -e "s/VAR/APCPsfc/" \
       -e "s/VA1/acp/" \
   $HOMEplot/plot_prcp_diff_org.gs > plot_prcp_diff.gs
   grads -bcl "plot_prcp_diff.gs"
   ...
   sed -e "s/test/$CDATE/" \
       ...
       -e "s/JDATE/$JDATE/" \
       -e "s/VAR/APCPsfc/" \
   $HOMEplot/plot_prcp_anf_org.gs > plot_prcp_anf.gs
   grads -bcl "plot_prcp_anf.gs"
   ...
   sed -e "s/test/$CDATE/" \
       ...
       -e "s/JDATE/$JDATE/" \
       -e "s/VAR/APCPsfc/" \
   $HOMEplot/plot_prcp_efi_org.gs > plot_prcp_efi.gs
   grads -bcl "plot_prcp_efi.gs"
...
do
   ...
   cp $COMIN/${px}geavg.t00z.pgrb2a.0p50_anf$FHR3d anvf$FHR3d.$id.${naefs_ver}
   cp $COMIN/${px}geefi.t00z.pgrb2a.0p50_bcf$FHR3d efif$FHR3d.$id.${naefs_ver}
   ...
   if [ -s geavg.t00z.pgrb2a.0p50_ana.ctl ]; then
    ...
    sed -e "s/test/$CDATE/" \
        ...
        -e "s/VAR/TMP2M/" \
        -e "s/VA1/t2m/" \
    $HOMEplot/plot_2vars_diff_org.gs > plot_3vars.gs
