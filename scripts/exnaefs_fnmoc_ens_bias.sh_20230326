...
###
  ...
  if [ ! -s $afile ]; then
    echo " There is no FNMOC Analysis data, Stop! for " ${aymd}${acyc}
  fi
...
###
  ...
  if [ ! -s $rfile_in ]; then
    echo " There is no CFS Reanalysis data, Stop! for " ${aymd}${acyc}
  fi
...
###
  ...
  if [ ! -s $nfile ]; then
    echo " There is no GEFS Analysis data, Stop! for " ${aymd}${acyc}
  fi
