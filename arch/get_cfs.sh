...
#HPSSOUT=/NCEPPROD/2year/hpssprod/runhistory
...
# file=$HPSSOUT/rh${YER}/${MON}/${PDY}/com2_gens_prod_gefs.${PDY}_${cyc}.ensstat.tar 
  file=$HPSSOUT/rh${YER}/${MON}/${PDY}/gpfs_hps_nco_ops_com_naefs_prod_gefs.${PDY}_${cyc}.pgrb2ap5.tar
...
#   hpsstar get $file ./pgrb2ap5/geavg.t${cyc}z.pgrb2a.0p50_mef$ifh
#   hpsstar get $file ./pgrb2ap5/geavg.t${cyc}z.pgrb2a.0p50_mecomf$ifh
    hpsstar get $file ./pgrb2ap5/geavg.t${cyc}z.pgrb2a.0p50.f$ifh
  done

  for ifh in $hourlist; do
    FDATE=`$NDATE +$ifh $CDATE`
    export YER=`echo $FDATE | cut -c1-4`
    export MON=`echo $FDATE | cut -c1-6`
    export PDY=`echo $FDATE | cut -c1-8`
    export cyc=`echo $FDATE | cut -c9-10`
    file=$HPSSOUT/rh${YER}/${MON}/${PDY}/gpfs_hps_nco_ops_com_naefs_prod_gefs.${PDY}_${cyc}.pgrb2ap5.tar
    echo $PDY $cyc
    echo $file
    mkdir -p ${COMOUT}/gefs.${PDY}/${cyc}/pgrb2ap5
    cd ${COMOUT}/gefs.${PDY}/${cyc}
    hpsstar get $file ./pgrb2ap5/gegfs.t${cyc}z.pgrb2ap5f000
  done
