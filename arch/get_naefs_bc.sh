...
while [ $iday -le $ndays ]; do
  ...
  mkdir -p ${COMOUT}/naefs.${PDY}/${cyc}/pgrb2ap5_bc
 
  cd ${COMOUT}/naefs.${PDY}/${cyc}

  file=$HPSSOUT/rh${YER}/${MON}/${PDY}/gpfs_hps_nco_ops_com_naefs_prod_naefs.${PDY}_${cyc}.pgrb2ap5_bc.tar

# hpsstar inx $file 
# hpsstar get $file 

  for ifh in $hourlist; do
    hpsstar get $file ./pgrb2ap5_bc/naefs_geavg.t${cyc}z.pgrb2a.0p50_bcf$ifh
  done
